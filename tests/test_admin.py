...
def test_card_statements_list_viewed_with_permission(superuser, card_statement_admin, request_get):
    request = request_get(reverse("admin:nubank_django_cardstatement_changelist"), user=superuser)
...
def test_account_statements_list_viewed_with_permission(superuser, account_statement_admin, request_get):
    request = request_get(reverse("admin:nubank_django_accountstatement_changelist"), user=superuser)
...
def test_card_statements_details_viewed_with_permission(superuser, card_statement_admin, request_get, nu_data_loader):
    ...
    request = request_get(
        reverse("admin:nubank_django_cardstatement_change", args=[statement.id]),
        user=superuser,
    )
...
def test_account_statements_details_viewed_with_permission(
    superuser, account_statement_admin, request_get, nu_data_loader
):
    nu_data_loader()
    statement = AccountStatement.objects.first()
    request = request_get(
        reverse("admin:nubank_django_accountstatement_change", args=[statement.id]),
        user=superuser,
    )
...
def test_account_import_action(superuser, account_statement_admin, request_get):
    ...
    request = request_get(
        reverse("admin:nubank_django_accountstatement_actions", args=["run_nuconta_import"]),
        user=superuser,
    )
    with mock.patch(
...
def test_card_import_action(superuser, card_statement_admin, request_get):
    ...
    request = request_get(
        reverse("admin:nubank_django_cardstatement_actions", args=["run_nubank_import"]),
        user=superuser,
    )
